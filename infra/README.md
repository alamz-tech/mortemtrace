...
| `seed_data.py` | `python -m infra.seed_data` (add `MORTEMTRACE_SEED_PUBLIC_DEMO=1` to also flag the org for judge/demo auto-join) | Thin wrapper around `seed/generate.py` - writes synthetic services, customers, incidents, evidence, and a committed timeline under the demo org. Idempotent (deterministic doc IDs). |
