...
| `create_service_accounts.sh` | `GOOGLE_CLOUD_PROJECT=mortemtrace-hackathon bash infra/create_service_accounts.sh`, once before the first `deploy.sh` that uses it - idempotent, safe to re-run | Creates two dedicated, least-privilege runtime service accounts (one per Cloud Run service) and grants each only the project roles its own code actually calls - Firestore/Pub/Sub/Vertex AI/Model Armor/tracing for ingest-api, Firestore/tracing only for console. Neither runs as the default Compute Engine service account (`roles/editor`, project-wide) anymore. `deploy.sh` requires both to exist first and fails its own precondition check otherwise. |
| `deploy.sh` | not run by this task - review then run manually | `gcloud run deploy` for the ingest API and console, `min-instances=0`, each with its own dedicated runtime service account (see `create_service_accounts.sh` above). Ingest runs real async dispatch (no `MORTEMTRACE_SYNC_DISPATCH`); sets `MORTEMTRACE_PUSH_AUDIENCE` from the service's own URL after the first deploy. |
